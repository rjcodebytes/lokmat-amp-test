...
export default function About(props: any) {
    ...
    useEffect(() => {
        if (!slug) {
            console.log("About: No slug provided");
            return;
        }
        
        console.log("About: Fetching data for slug:", slug);
        console.log("About: API URL:", customPageURL);
        
        cachedAxiosGet(customPageURL, axiosConfig)
            .then((result: any) => {
                console.log("About: API Response:", result);
                const data = result?._data ?? result;
                console.log("About: Extracted data:", data);
                
                // Handle both array and object responses
                if (Array.isArray(data)) {
                    // If array, take first item or empty object
                    setCustomPages(data.length > 0 ? data[0] : null);
                } else if (data && typeof data === 'object') {
                    // If object, use it directly
                    setCustomPages(data);
                } else {
                    console.warn("About: Unexpected data format:", data);
                    setCustomPages(null);
                }
            })
            .catch((error) => {
                console.error("About: Error fetching custom page data:", error);
                setCustomPages(null);
            });
